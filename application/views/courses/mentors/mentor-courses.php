    ...
    <div class="card">
        <div class="card-body">
            <div class="mb-24 flex-between gap-16 flex-wrap-reverse">
                ...
                <a href="<?=base_url('create-course')?>" class="btn btn-main rounded-pill py-7 flex-align gap-4 fw-normal">
            ...
            <div class="tab-content" id="pills-tabContent">
                <div class="tab-pane fade show active" id="pills-onGoing" role="tabpanel" aria-labelledby="pills-onGoing-tab" tabindex="0">
                    <div id="courses-list" class="row g-20">
...
document.addEventListener('DOMContentLoaded', function() {
                ...
                <div class="card border border-gray-100">
                    <div class="card-body p-8">
                        ...
                        <div class="p-8">
                            ...
                            <div class="flex-align gap-8 flex-wrap mt-16">
                                <img src="<?= base_url('assets/images/thumbs/user-img.png') ?>" class="w-32 h-32 rounded-circle object-fit-cover" alt="User Image">
                                <div>
                            ...
                            <div class="mt-16">
                                <span class="badge bg-${course.is_published == 1 ? 'success' : 'warning'}-50 text-${course.is_published == 1 ? 'success' : 'warning'}-600">${course.is_published == 1 ? 'Published' : 'Draft'}</span>
                            </div>
                            <a href="${course.url}" class="btn btn-outline-main rounded-pill py-9 w-100 mt-24">View Details</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    function loadCourses() {
        showLoading();
        fetch('<?= base_url('mentor-courses/getMentorCourses') ?>')
            .then(response => response.json())
            .then(data => {
                hideLoading();
                if (data.success) {
                    coursesList.innerHTML = data.data.map(course => renderCourse(course)).join('');
                } else {
                    showError();
                }
            })
