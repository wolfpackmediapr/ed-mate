<div id="step-33-content" class="card step-content" style="display:none;">
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.resUploadedFiles && response.resUploadedFiles.length > 0) {
                    // Clear the previous list (optional)
                    // $('#res-uploaded-video-list').empty();

                    // Loop through each uploaded file and check its type
                    response.resUploadedFiles.forEach(function(file) {
                        if (file.filetype === 'pdf') { // Ensure you check filetype for each file
                            // Dynamically populate file details for PDFs
                            var fileHTML = `
                    <div class="flex-between gap-8">
