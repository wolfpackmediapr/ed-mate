...
<div class="dashboard-body">
    ...
    <div class="row gy-4">
        <div class="col-md-8">
            <!-- Course Banner Start -->
            <?php if ($course): ?>
            <div class="course-banner mb-4" style="background-image: url('<?= htmlspecialchars($course->thumbnail_image) ?>'); height: 250px; background-size: cover; background-position: center;">
              <div class="overlay d-flex align-items-center h-100" style="background: rgba(0,0,0,0.4);">
            ...
            <div id="display-selected-course" class="card">
                <div class="card-body p-lg-20 p-sm-3">
                    <div class="flex-between flex-wrap gap-12 mb-20">
                        <div>
                            <h3 class="mb-4"><?= $course ? $course->course_title : 'Course Not Found' ?></h3>
                            <p class="text-gray-600 text-15"><?= $createdBY ? $createdBY->username : 'Unknown Instructor' ?></p>
                        </div>

                        <div class="flex-align flex-wrap gap-24">
                            <span class="py-6 px-16 bg-main-50 text-main-600 rounded-pill text-15"><?= $category ? $category->category_name : 'Uncategorized' ?></span>
                    ...
                    $video_path = !empty($resources) ? $resources[0]->path : '';
                    $play_path = !empty($video_path) ? base_url() . 'uploads/resources/' . $video_path : '';
                    ?>

                    <div class="rounded-16 overflow-hidden">
                        <video id="player" class="video-js vjs-big-play-centered w-100" controls preload="auto" data-setup='{}' poster="<?= $course ? htmlspecialchars($course->thumbnail_image) : base_url() . 'assets/images/default-course.png' ?>">
                            <source src="<?= $course ? htmlspecialchars($course->video_url) : '' ?>" type="video/mp4" />
                        </video>
                    </div>
                    ...
                    <div class="mt-24">
                        <div class="mb-24 pb-24 border-bottom border-gray-100">
                            ...
                            <p class="text-gray-300 text-15"><?= !empty($lessons) ? $lessons[0]->content : 'No content available' ?></p>
                        </div>
                        <div class="mb-24 pb-24 border-bottom border-gray-100">
                            <h5 class="mb-12 fw-bold">Description</h5>
                            <p class="text-gray-300 text-15 mb-8"><?= $course ? $course->description : 'No description available' ?></p>
                        </div>
                        <div class="">
                            <h5 class="mb-12 fw-bold">Instructor</h5>
                            <div class="flex-align gap-8">
                                <img src="<?= base_url() ?>assets/images/thumbs/mentor-img1.png" alt="" class="w-44 h-44 rounded-circle object-fit-cover flex-shrink-0">
                                <div class="d-flex flex-column">
                                    <h6 class="text-15 fw-bold mb-0"><?= $createdBY ? $createdBY->username : 'Unknown Instructor' ?></h6>
                                    <span class="text-13 text-gray-300">Course Instructor</span>
        ...
        <div class="col-md-4">
            ...
            <div class="card">
                <div class="card-body p-0">
                    <?php foreach ($lessons as $index => $lesson) {
                        ...
                        $totalDurationFormatted = number_format($totalDuration, 1) . ' min';
                    ?>
                        <div class="course-item">
                            <button type="button" class="course-item__button <?= $index === 0 ? 'active' : '' ?> flex-align gap-4 w-100 p-16 border-bottom border-gray-100"
                                ...
                                <span class="d-block text-start">
                                    ...
                                    <span class="d-block text-15 text-gray-300"><?= $completedResources . ' / ' . $totalResources . ' | ' . $totalDurationFormatted ?></span>
                                </span>
                            ...
                            <div class="course-item-dropdown <?= $index === 0 ? 'active' : 'hidden' ?> border-bottom border-gray-100">
                                <ul class="course-list p-16 pb-0">
                                    <?php if (!empty($resources)): ?>
                                        <?php foreach ($resources as $resIndex => $resource): 
                                            $filePath = FCPATH . 'uploads/resources/' . $resource->path;
                                            $get_duration = get_video_details($filePath);
                                            $class = ($is_paid || $resIndex < 2) ? 'active' : 'disabled';
                                            $is_active = $is_paid || $resIndex < 2;
                                        ?>
                                            <li class="course-list__item flex-align gap-8 mb-16 <?= $class ?>">
                                                ...
                                                <div class="w-100">
                                                    ...
                                                    <?php else: ?>
                                                        <span class="text-gray-300 fw-medium d-block">
                                                            <?= ($resIndex + 1) . '. ' . htmlspecialchars($resource->title) ?>
                                                            <span class="text-gray-300 fw-normal d-block"><?= $get_duration['duration']; ?> min</span>
                                                        </span>
                                                    <?php endif; ?>
            ...
            <div class="card mt-24">
                <div class="card-body">
                    ...
                    <div class="rounded-16 overflow-hidden">
                        <video id="featuredPlayer" class="player" playsinline controls data-poster="<?= base_url() ?>assets/images/thumbs/featured-course.png">
    ...
    <div class="accordion mb-4" id="lessonsAccordion">
      <?php foreach ($lessons as $i => $lesson): ?>
        <div class="accordion-item">
          ...
          <div id="collapse<?= $i ?>" class="accordion-collapse collapse <?= $i === 0 ? 'show' : '' ?>" aria-labelledby="heading<?= $i ?>" data-bs-parent="#lessonsAccordion">
            <div class="accordion-body">
              <?= nl2br(htmlspecialchars($lesson->content ?? $lesson->lesson_description ?? '')) ?>
              <!-- Add resource links/videos here if needed -->
            </div>
          </div>
        </div>
      <?php endforeach; ?>
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    <div class="rounded-16 overflow-hidden">
                        <video id="player" class="video-js vjs-big-play-centered w-100" controls preload="auto" data-setup='{}' poster="<?= base_url() ?>uploads/courses/${response.course.thumbnail_image}">
                    ...
                    // Get the newly rendered video element by ID
                    let video = document.getElementById('player');

                    // Set up event listeners to track the video progress
                    video.addEventListener('timeupdate', function() {
                        let duration = video.duration; // Total video duration
                        let currentTime = video.currentTime; // Current time watched
                        // Calculate the percentage watched
                        let percentWatched = (currentTime / duration) * 100;

                        // Debugging: Check if timeupdate is firing
                        console.log('timeupdate fired', currentTime, duration, percentWatched);

                        // Send progress to the server
                        updateVideoProgress(resource_id, course_id, lesson_id, percentWatched);
                    });
...
<script>
    function updateVideoProgress(resource_id, course_id, lesson_id, percentWatched) {
        // Debugging: Check if updateVideoProgress is called
        console.log('updateVideoProgress called', resource_id, course_id, lesson_id, percentWatched);
// return;
        $.ajax({
            type: "POST",
            url: "<?= base_url('StudentCoursesController/updateVideoProgress'); ?>", // Your endpoint for tracking progress
            data: {
                resource_id: resource_id,
                course_id: course_id,
                lesson_id: lesson_id,
...
<!-- Add Video.js JS CDN before </body> or at the end of the file -->
