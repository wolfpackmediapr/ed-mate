    ...
    function renderLessonRow(lesson) {
        return `
            <tr>
                ...
                <td><span class="h6 mb-0 fw-medium text-gray-300">${lesson.lesson_id}</span></td>
                <td><div class="flex-align gap-8"><span class="h6 mb-0 fw-medium text-gray-300">${lesson.lesson_title}</span></div></td>
                <td><span class="h6 mb-0 fw-medium text-gray-300">${lesson.createdAt}</span></td>
        ...
        fetch('<?= base_url('api/lessons') ?>')
            ...
            .then(data => {
                ...
                if (Array.isArray(data)) {
                    lessonsTbody.innerHTML = data.map(renderLessonRow).join('');
                } else if (data.data && Array.isArray(data.data)) {
                    lessonsTbody.innerHTML = data.data.map(renderLessonRow).join('');
                } else {
                    showError();
                }
            })
