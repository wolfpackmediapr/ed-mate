...
class User extends MY_Controller
{
	...
	public function index()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$response = $this->login_api($username, $password);

			if (isset($response['error'])) {
				// Handle error
				$this->output->set_status_header(500)->set_output(json_encode(array('error' => $response['error'])));
			} else {
				if (isset($response['user_data'])) {
					// Extract the token from 'user_data'
					$token = $response['user_data']['token'];
					$this->session->set_userdata('profile_data', $response['user_data']);

					// Store the token in session (optional)
					$this->session->set_userdata('token', $token);
					if ($response['status'] == 200) {
						redirect('dashboard');
					}
				} else {
					// Handle missing 'user_data'
					$this->output->set_status_header(500)->set_output(json_encode(array('error' => 'User data not found in response.')));
				}
			}
		}
	}
	...
	public function profile()
	{
		...
		$data['page_name'] = 'View Vendor Profile';
		$order_data = array();
		$response = $this->profileApi();
		// print_r($response);
		// 	die;
		if (isset($response['data']) && isset($response['data'])) {
			...
			// $this->output->set_content_type('application/json')->set_output(json_encode(array('order_data' => $order_data)));
		} else {
			redirect(base_url());
			// Handle missing 'data' or 'order_data'
			// $this->output->set_status_header(500)->set_output(json_encode(array('error' => 'Profile data not found in response.')));
		}

		$var['content'] = $this->load->view('pages/profile', $data, true);
