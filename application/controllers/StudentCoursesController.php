...
class StudentCoursesController extends CI_Controller
{
    ...
    public function courseDetails($course_id)
    {
        ...
        if (!empty($data['courses'])) {
            ...
            // Log the course view
            $user_id = $this->session->userdata('user_id');
            $this->ActivityLog_model->log_activity($user_id, 'course_view', 'course', $course_id);
            ...
            $data['lessons'] = $this->getLessonsByCourseId($course_id);

            // Get resources for this course
            $data['resources'] = $this->getResourcesByCourseId($course_id);

            // Check if user has paid for this course
            $user_id = $this->session->userdata('user_id');
            $data['is_paid'] = $this->checkIfUserPaid($user_id, $course_id);

            // Get creator info
            $data['createdBY'] = $this->common_model->select_where_return_row('*', 'users', array('user_id' => $data['course']->created_by));
        } else {
            // Set default values when course is not found
            $data['course'] = null;
            $data['category'] = null;
            $data['lessons'] = [];
            $data['resources'] = [];
            ...
            // Set flash message
            $this->session->set_flashdata('error', 'Course not found or is not published.');
    ...
    // Method to fetch lessons for a course
    private function getLessonsByCourseId($course_id)
    {
    ...
    private function getResourcesByCourseId($course_id)
    {
        $this->db->select('resources.*');
        $this->db->from('resources');
        $this->db->join('lessons', 'lessons.lesson_id = resources.lesson_id');
        $this->db->where('lessons.course_id', $course_id);
        $this->db->where('lessons.isDeleted', 0);
        $query = $this->db->get();
        return $query->result();
    }

    // Method to check if user has paid for the course
    private function checkIfUserPaid($user_id, $course_id)
    {
        $this->db->where('user_id', $user_id);
        $this->db->where('course_id', $course_id);
        $this->db->where('is_paid', 1);
        $query = $this->db->get('course_pricing');
    ...
    public function getStudentCourses()
    {
        ...
        // Get courses where payment is done (is_paid = 1)
        $courses = $this->db
            ->select('courses.*, categories.category_name, users.username, course_pricing.is_paid')
            ->from('courses')
            ->join('categories', 'courses.category_id = categories.category_id', 'left')
            ->join('users', 'courses.created_by = users.user_id', 'left')
            ->join('course_pricing', 'course_pricing.course_id = courses.course_id', 'inner')
            ->where('courses.is_published', 1)
            ->where('course_pricing.is_paid', 1)
            ->where('course_pricing.user_id', $user_id)
            ->order_by('courses.course_id', 'DESC')
        ...
        $formatted_courses = array_map(function($course) {
            return [
                ...
                'url' => base_url('student-course-details/' . $course->course_id)
            ];
        }, $courses);
